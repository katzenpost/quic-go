...
var _ = Describe("SentPacketHandler", func() {
	...
	JustBeforeEach(func() {
		...
		handler = newSentPacketHandler(42, protocol.InitialPacketSizeIPv4, protocol.MaxPacketBufferSize, rttStats, false, false, perspective, nil, utils.DefaultLogger)
		streamFrame = wire.StreamFrame{
			StreamID: 5,
			Data:     []byte{0x13, 0x37},
		}
	...
	Context("amplification limit, for the server, with validated address", func() {
		JustBeforeEach(func() {
			rttStats := utils.NewRTTStats()
			handler = newSentPacketHandler(42, protocol.InitialPacketSizeIPv4, protocol.MaxPacketBufferSize, rttStats, true, false, perspective, nil, utils.DefaultLogger)
		})
